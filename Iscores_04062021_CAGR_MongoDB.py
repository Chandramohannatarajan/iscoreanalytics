...
dataa = data.merge(df, on=[ 'REGISTRATION_NUMBER','COMPANY_NAME','INDUSTRY_TYPE','Year'])
dataa.to_csv("E://DG/DG_DB/main/ALL/industrywise/dummy/cagr/all_scores_final.csv", index=False)
...
df.to_csv(r'E://DG/DG_DB/main/ALL/industrywise/dummy/cagr/CAGR.csv',index = False, na_rep = 'N/A')
df_cagr_per.to_csv(r'E://DG/DG_DB/main/ALL/industrywise/dummy/cagr/CAGR_per.csv',index = False, na_rep = 'N/A')
...
CAGR_mark = dataa.groupby(['REGISTRATION_NUMBER','COMPANY_NAME','INDUSTRY_TYPE','Year'], as_index=False)
...
conditions2_at = [
                (df['CAGR']>df_stats['CAGR'][3])&(df['CAGR'] <= df_stats['CAGR'][4]),
                (df['CAGR']>df_stats['CAGR'][4])&(df['CAGR'] <= df_stats['CAGR'][5]),
                (df['CAGR']>df_stats['CAGR'][5])&(df['CAGR'] <= df_stats['CAGR'][6]),
                (df['CAGR']>df_stats['CAGR'][6])&(df['CAGR'] <= df_stats['CAGR'][7]),   
                ]
values2_at = ['1', '2', '3', '4']
...
df_iserve=df.Iservice.describe()
...
df.iserve=df_iserve.values.tolist
...
print(green_report[['COMPANY_NAME','Year','CAGR']].head(10))
...
print(amber_report[['COMPANY_NAME','Year','CAGR']].head(10))
...
red_report=df[red]
print(red_report[['COMPANY_NAME','Year','CAGR']].head(10))
...
print(black_report[['COMPANY_NAME','Year','CAGR']].head(10))
