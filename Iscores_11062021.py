...
print(green_report[['COMPANY_NAME','Year','CAGR']].head(5))
...
print(amber_report[['COMPANY_NAME','Year','CAGR']].head(5))
...
red_report=df[red]
print(red_report[['COMPANY_NAME','Year','CAGR']].head(5))
...
print(black_report[['COMPANY_NAME','Year','CAGR']].head(5))
...
for i,value in enumerate(df_all_year):
    data_year=dataa[dataa['Year'] == value].to_dict('records')
...
df_all_type=dataa['INDUSTRY_CODE'].unique().tolist()
for k,value_year in enumerate(df_all_year):
    for l,value_type in enumerate(df_all_type):
        data_year_ind=dataa[(dataa['INDUSTRY_CODE'] == value_type) & (data['Year'] == value_year)].to_dict('records')
        ...
        year_industry = year_industry[['REGISTRATION_NUMBER','COMPANY_NAME','INDUSTRY_TYPE','Year','PRETAX_PROFIT_PERCENTAGE', 'CURRENT_RATIO',
                                       ...
                                       'RETURN_NET_ASSETS', 'TOTAL_DEBT_RATIO', 'EBITDA','Z', 'CAGR']]
...
#export to csv
iscores_z_cagr.to_csv( "C://Users/44740/Machine learning/DG/Credit_Score_Analysis-master/05062021/iscore/iscores_z_cagr.csv", index=False)


# In[43]:


iscores_z_cagr.count()
...
all_scores_final = a.merge(b, left_index=True, right_index=True,
             how='outer', suffixes=('', '_DROP')).filter(regex='^(?!.*_DROP)')
all_scores_final.to_csv("C://Users/44740/Machine learning/DG/Credit_Score_Analysis-master/05062021/all_scores_05062021.csv", index=False)
...
stage_12_table=db['all_scores_05062021']


# In[53]:


stage_12_table.insert_many(all_scores_final_dict)
