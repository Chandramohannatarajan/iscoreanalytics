...
root_data_cagr=root_data_cagr.sort_values(by="YEAR",ascending=True)


# In[102]:


list_year_type= [v for k, v in root_data_cagr.groupby(['INDUSTRY_TYPE','YEAR'])]
...
cagr_mean=[]
for i in list_year_type:
    type_ind=i['INDUSTRY_TYPE'].unique().tolist()
    type_year=i['YEAR'].unique().tolist()
    mean_cagr=i['CAGR'].mean()
    cagr_mean.append((type_ind[0],type_year[0],mean_cagr))


# In[163]:


list_allcagr = DataFrame (cagr_mean,columns=['INDUSTRY_TYPE','YEAR','CAGR'])
list_allcagr
...
df_all_type=list_allcagr["INDUSTRY_TYPE"].unique().tolist()
for l,value_type in enumerate(df_all_type):
    ind_type =  list_allcagr['INDUSTRY_TYPE']==str(value_type)
    data = list_allcagr[ind_type]
